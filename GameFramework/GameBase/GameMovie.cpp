...
GameMovie* GameMovie::MainMoviePlay = nullptr;

HRESULT GameMovie::Play(const GameString& Path)
{
    // 
    if (S_OK != MainMoviePlay->OpenFile(Path))
    {
        MSG_ASSERT_CODE(L"무비파일 오픈에 실패했습니다.");
    }
...
HRESULT GameMovie::Pause()
{
    ...
    {
        MSG_ASSERT_CODE(L"메인 무비 재생기가 존재하지 않습니다 윈도우를 만드세요");
    }

    return S_OK;
}

HRESULT GameMovie::Stop()
{
    if (nullptr != MainMoviePlay)
    {
...
bool GameMovie::IsPlay()
{
    return MainMoviePlay->State() == PlaybackState::STATE_RUNNING;
}

bool GameMovie::IsPlayE()
{
    return MainMoviePlay->State() != PlaybackState::STATE_STOPPED;
}

//void CHRGAMEMOVIE::PrintMovie()
//{
//    MainMoviePlay->Repaint();
//}

GameMovie::GameMovie(HWND hwnd) :
...
HRESULT GameMovie::OpenFile(PCWSTR pszFileName)
{
...
HRESULT GameMovie::HandleGraphEvent(GraphEventFN pfnOnGraphEvent)
{
    if (!m_pEvent)
    {
        return E_UNEXPECTED;
...
HRESULT GameMovie::UpdateVideoWindow(const LPRECT prc)
{
    if (m_pVideo)
    {
...
HRESULT GameMovie::Repaint(HDC _DC)
{
    if (m_pVideo)
    {
...
HRESULT GameMovie::DisplayModeChanged()
{
    if (m_pVideo)
    {
...
HRESULT GameMovie::InitializeGraph()
{
    TearDownGraph();
...
HRESULT GameMovie::CreateVideoRenderer()
{
    HRESULT hr = E_FAIL;
...
HRESULT GameMovie::RenderStreams(IBaseFilter* pSource)
{
